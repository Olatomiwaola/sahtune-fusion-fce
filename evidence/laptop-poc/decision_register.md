...
**Sprint 14 close 2026-07-08 (append-only; lead decision in-chat) — M7 two-layer harness result:**

- FCE-DR-POC-005 EVALUATED (docs/16 criteria 1-10): 9/10 MET; criterion 4 PARTIAL (GDR-010 no-bypass rejection test deferred). Outcome label, verbatim per docs/16: "engineering progress; PoC validation gate not passed". Basis: single sealed held-out run under proj-baseline@0.2.0 (sha 6a830b24…c53502; aggregate 059829241e…954ed63e), 9/10 oracle MATCH, 1 disclosed NEGATIVE (stale -> no RC-004). Full record: EVD-M7 (evidence/laptop-poc/heldout_eval_report.md); Layer 1 code-correctness = 126 passed / 1 skip.
- FCE-DR-SCH-004 D5 duplicate-object_id: IMPLEMENTED this sprint — additive per-run, batch-level dedup wrapper around the untouched single-object evaluator; a repeated object_id in a run is quarantined fail-closed on the RC-001 path with a duplicate_object_id detection flag naming the colliding id. No new reason code; no change to unrelated validation. Held-out re-run: MATCH. Closes the M6 "duplicate-ID handling pending" gap.
- Carried honest negative (stale -> no RC-004): the held-out stale variant (acquisition_timestamp RFC3339 2000-01-01, no injected tick) does not trigger the tick-based RULE-ING-011. FCE-REQ-ING-011 partially demonstrated: tick-based RC-004 emission works (Layer 1, TST-UNT-030); RFC3339/wall-clock freshness deferred, tied to H4 (trusted/attested time), unclaimed. Lead decision: do not fix in Sprint 14; recorded verbatim in EVD-M7.
- RT-M7S14-01 (red-team finding, carried tech-debt): dual freshness model (_stale + RULE-ING-011) left in place under the additive constraint; RULE-ING-011 fires first, deterministic; consolidation (retire _stale) deferred to follow-on TRL work.
- Carried to TRL 4-5 / follow-on: H4 (RFC3339/trusted-time freshness), GDR-010 no-bypass rejection test, RT-M7S14-01 consolidation.
