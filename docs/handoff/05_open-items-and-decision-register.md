...
| FU-M2S4-1 | Author the enumerated project-taxonomy registry in `docs/07` (classification, domain, caveat, modality value families) and add a guard that the calibration `taxonomy.json` equals the docs/07 families. **UNBLOCKED 2026-07-04** (decision #2 approved): docs/07 stub landed (D6, `docs/07` v1); registry authoring in progress (chat-side, fail-closed hold pending fixture values + SHA-256 verification); interim authoritative source = EVD-M2 hash-pinned fixture. Consumes OPEN-02 / decision #2. | policy-engineer | in progress (M3 Sprint 6) |
