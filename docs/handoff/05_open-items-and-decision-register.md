...
| FU-M4S8-1 | Align docs/08 downgrade requiredness and policy-decision detail strictness (Sprint 8 verification finding). (1) Amend the docs/08 requiredness matrix to add the missing `downgrade` event row: source_object_ids ≥1; output_object_id required; policy_bundle_version semver; policy_rule_ids ≥1; event_detail requires authority + transformation-proof reference. (2) Align policy-decision `event_detail` strictness with docs/07 D4 atomic emission: `pip_attributes_consumed`, `detection_flags`, `deterministic_evaluation` required for policy-decision records unless a successor DR says otherwise. (3) Update writer validation/tests if needed. Sprint 8 PoC behaviour (minimal downgrade requiredness by judgment; optional policy-decision detail) is recorded in EVD-M4 and stands until this closes. | audit-forensics-engineer | OPEN — M5 Sprint 9 open |
