...
| 11 | M6 | Finalize four scenario specs (Joint ISR, Maritime, Tactical Edge, UAV) with embedded conflicts; confirm source choices under OPEN-04 | Scenario library, trim protocol, split plan, expected dispositions | Chat | 0.5–1 day | — | OPEN 2026-07-07 — blocker cleared by leadership decision #6 (USGS + Sentinel-2 STAC fixed); remainder (trim limits, split, guard list) confirmed at M6 open per docs/16 (lead concurrence 2026-07-07); scenario baseline = docs/09_synthetic-dataset-plan.md v0 (citation resolved live) |
