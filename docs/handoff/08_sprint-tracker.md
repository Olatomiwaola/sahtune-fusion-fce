...
| 14 | M7 | Implement and run the two-layer harness: code-correctness tests, then the sealed held-out validation run; report negative results verbatim | Runnable harness, guard rejection tests, code-correctness report, held-out validation report, coverage report (EVD-M7) | Claude Code | 1–2 days | GATE-D closes when Sprints 10 + 14 done | Pending Sprint 14 (two-layer harness build + sealed held-out run + EVD-M7); Sprints 10 and 12 DONE; opens after FCE-DR-POC-005 ratification and the Scenario 4 covers() resolution (bundle read-back before any held-out read). GATE-D closes when Sprints 10 + 14 done. |
